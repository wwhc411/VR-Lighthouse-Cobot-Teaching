...
/// MoveJ 批量脚本生成器
/// 
/// 功能：将完整轨迹转换为URScript脚本（使用movej指令），一次性发送执行
/// 
/// 与Servoj回放的区别：
///   - Servoj：实时伺服控制，每帧精确跟踪，适合高精度轨迹还原
///   - MoveJ：关节空间插值运动，每个点之间平滑过渡，适合点到点运动
/// 
/// MoveJ特点：
///   - 关节空间轨迹线性，笛卡尔空间轨迹不一定是直线
///   - 支持混合半径(blend radius)实现平滑连续运动
///   - 运动速度和加速度在关节空间均匀
/// 
/// 脚本结构：
///   def trajectory_replay_movej():
///     qnear = get_actual_joint_positions()
///     P0 = p[x, y, z, rx, ry, rz]
///     P1 = p[...]
///     ...
///     movej(get_inverse_kin(P0, qnear=...), a=..., v=..., t=..., r=blend_radius)
///     movej(get_inverse_kin(P1, qnear=...), a=..., v=..., t=..., r=blend_radius)
///     ...
///     stopl(5)
///   end
///   trajectory_replay_movej()
/// 
/// 更新日期: 2026-02-03
/// </summary>
public static class RigidBodyMovejScriptGenerator
{
    #region 脚本参数结构
    ...
    /// </summary>
    [Serializable]
    public struct MovejScriptParameters
    {
        [Tooltip("关节加速度(rad/s²)，推荐0.5-2.0")]
        public float Acceleration;

        [Tooltip("关节速度(rad/s)，推荐0.3-1.0")]
        public float Velocity;

        [Tooltip("运动时间(s)，>0时忽略a和v参数，设为0使用a/v控制")]
        public float Time;

        [Tooltip("混合半径(m)，用于平滑连接多个movej指令\n" +
                 "0=精确到达每个点\n" +
                 ">0=在混合半径内开始下一段运动（更平滑）")]
        ...
        /// 默认参数
        /// </summary>
        public static MovejScriptParameters Default => new MovejScriptParameters
        {
            Acceleration = 1.0f,
            Velocity = 0.5f,
            Time = 0f,
        ...
        {
            if (Acceleration < 0.1f || Acceleration > 10f)
            {
                errorMessage = $"加速度超出范围(0.1-10 rad/s²): {Acceleration}";
                return false;
            }

            if (Velocity < 0.1f || Velocity > 3.14f)
            {
                errorMessage = $"速度超出范围(0.1-3.14 rad/s): {Velocity}";
                return false;
            }
    ...
    public static ScriptGenerationResult GenerateTrajectoryScript(
        ...
        MovejScriptParameters parameters,
        bool useTcpMode = false)
    {
        ...
        try
        {
            ...
            {
                Debug.LogWarning($"[MoveJ脚本] 点数({effectivePoints})超过限制({MAX_POINTS_PER_SCRIPT})，建议增大采样步长或分段发送");
            }
            ...
            // ========== 脚本头部 ==========
            sb.AppendLine("def trajectory_replay_movej():");
            ...
            for (int i = 0; i < frameCount; i += parameters.PointStep)
            {
                ...
                if (!frame.IsPositionValid())
                {
                    if (EnableDebugLog)
                    {
                        Debug.LogWarning($"[MoveJ脚本] 帧{i}位置无效，跳过");
                    }
                    continue;
                }
            ...
            for (int i = 0; i < processedIndices.Count; i++)
            {
                ...
                // 生成movej命令
                // 格式: movej(get_inverse_kin(p[x,y,z,rx,ry,rz], qnear=qnear), a=..., v=..., t=..., r=...)
                sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
                    "  movej(get_inverse_kin(P{0}, qnear=get_actual_joint_positions()), a={1:F4}, v={2:F4}, t={3:F2}, r={4:F4})",
                    idx,
            ...
            sb.AppendLine("end");
            sb.AppendLine("");
            sb.AppendLine("trajectory_replay_movej()");  // 调用函数执行

            // 计算预估时长
            // MoveJ时长难以精确估计，这里给一个粗略估计
            ...
            if (EnableDebugLog)
            {
                Debug.Log($"[MoveJ脚本] 生成完成: {pointIndex}点, {result.ScriptSizeBytes}字节, 预计{result.EstimatedDurationSeconds:F1}秒");
            }
        ...
        catch (Exception ex)
        {
            ...
            Debug.LogError($"[MoveJ脚本] {result.ErrorMessage}\n{ex.StackTrace}");
            return result;
        }
    }
    ...
    public static bool SaveScriptToFile(string script, string filePath)
    {
        try
        {
            ...
            Debug.Log($"[MoveJ脚本] 脚本已保存到: {filePath}");
            return true;
        }
        catch (Exception ex)
        {
            Debug.LogError($"[MoveJ脚本] 保存失败: {ex.Message}");
            return false;
        }
    }
    ...
    public static bool SendScriptToUR(string script)
    {
        ...
        {
            Debug.LogError("[MoveJ脚本] 脚本为空，无法发送");
            return false;
        }
        ...
        try
        {
            ...
            Debug.Log($"[MoveJ脚本] 脚本已发送到UR，大小: {scriptBytes.Length} 字节");
            return true;
        }
        catch (Exception ex)
        {
            Debug.LogError($"[MoveJ脚本] 发送失败: {ex.Message}");
            return false;
        }
    }
    ...
    /// 处理Tracker位姿（坐标转换）
    /// </summary>
    private static void ProcessTrackerPose(FrameData frame, MovejScriptParameters parameters,
