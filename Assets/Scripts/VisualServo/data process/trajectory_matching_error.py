...
CONFIG = {
    ...
    'metrics': {
        'compute_apd':         False,  # 1. 平均垂直距离 (APD)   - 较慢 O(N×M)
        'compute_frechet':     False,  # 2. 离散Fréchet距离      - 最慢 O(N×M)²
        'compute_endpoint':    False,  # 3. 端点误差             - 瞬时
        'compute_jerk':        True,   # 4. Jerk曲率变化率       - 快速 O(N)
        'compute_orientation': True,   # 5. 姿态角度误差         - 快速 O(N)
    },
}
...
if __name__ == "__main__":
    ...
    # 示教轨迹（参考轨迹）
    teach_csv = r"E:\Unity cangku\lighthouse_3.4（走数据备份）\Assets\StreamingAssets\工件\TRACKER - tcp_transformed.csv"
    # 回放轨迹（待评估轨迹）
    replay_csv = r"E:\Unity cangku\lighthouse_3.4（走数据备份）\Assets\StreamingAssets\工件\TCP - tcp.csv"
    # ========== 执行分析 ==========
