...
# ========== 文件路径 ==========
SOURCE_CSV = r"C:\Users\15421\Desktop\lighthouse_1.12\Assets\StreamingAssets\TrackerRecordings\tracker6.csv"
TARGET_CSV = r"C:\Users\15421\Desktop\lighthouse_1.12\Assets\StreamingAssets\TrackerRecordings\tcp6.csv"

# ========== 配准方法选择 ==========
# "kabsch"           - Kabsch算法（点对点对应，最简单直接）
# "kabsch_icp"       - Kabsch+ICP混合精配准
# "kabsch_segmented" - ⭐⭐ 分段Kabsch（推荐！堰死RMSE<3）
...
ADAPTIVE_SPATIAL_RESAMPLE = {
    ...
    "delta_d": 2.0,                    # ⭐ 采样步长门限(mm)，相邻采样点最小弦距
                                     #   0.5 = 高密度（保留更多细节）
                                     #   1.0 = 推荐值（平衡抗噪与细节）
                                     #   2.0 = 低密度（强抗噪，可能丢失细节）
    ...
    "noise_level": 0.9,              # 测量噪声水平(mm)（adaptive_spatial专用）
    "noise_suppression_factor": 2.0,   # 噪声抑制因子（1.5推荐）
    # --- 公共参数 ---
...
MLS_PARAMS = {
    ...
    "loocv_candidates": [0.028],  # LOOCV候选带宽
    "grid_size": 100,                 # 预计算网格点数（200~500）
...
RANSAC_KABSCH_PARAMS = {
    ...
    "min_sample_points": 3,         # 最小采样点数（3个点确定刚体变换）
}

# ========== ⭐⭐ 预平滑参数（弧长重采样前，消除噪声对弧长计算的影响）==========
# 💡 adaptive_spatial 方法需要预平滑（建议开启）
#    chord_spatial 方法自身解决海岸线悖论，无需预平滑（可关闭）
PRE_SMOOTH = {
    "enable": True,                # ⭐ chord_spatial时建议关闭，adaptive_spatial时建议开启
    "method": "gaussian",           # "gaussian" 或 "bspline"
    "gaussian_sigma": 5,          # ⭐ 中等平滑（原0.8，针对10%噪声提升到1.5）
    "bspline_smoothing": 3.0,       # B-Spline平滑因子
    "bspline_k": 3,                 # B-Spline次数
}
...
POST_SMOOTH = {
    ...
    "gaussian_sigma": 3.0,          # ⭐ 后平滑可以稍强一些
    "bspline_smoothing": 5,       # B-Spline平滑因子（1-100）
...
def adaptive_spatial_resample(points, noise_level=0.5, noise_suppression_factor=2.0,
    ...
    # 4. 实际步长（考虑边界约束后）
    actual_delta_d = total_length / num_samples
    
    # 5. 生成固定步长的采样位置
    sample_arc_positions = np.arange(num_samples) * actual_delta_d
    ...
    for i in range(3):
        ...
        resampled[:, i] = f(sample_arc_positions)
    ...
    sampling_info = {
        ...
        'delta_d_to_noise_ratio': actual_delta_d / noise_level,
