...
DEFAULT_TRANSFORM_PATH = r"C:\Users\15421\Desktop\lighthouse_1.12\Assets\StreamingAssets\TrackerRecordings\mls_transform_robot.json"

# ========== ⭐⭐⭐ 空间重采样参数（与tunable_registration.py保持一致！）==========
ADAPTIVE_SPATIAL_RESAMPLE = {
    # --- 新版 chord_spatial 参数（弦距采样，解决海岸线悖论）---
    "delta_d": 3.6,                  # ⭐ 弦距步长门限(mm)
    "densify_factor": 50,            # ⭐ 插值加密倍数
    # --- 旧版 adaptive_spatial 参数（弧长插值，需配合预平滑）---
    "noise_level": 0.9,              # 噪声水平估计(mm)
    "noise_suppression_factor": 2.0, # 噪声抑制系数
    # --- 共享参数 ---
...
# ========== ⭐⭐ 预平滑参数 ==========
# 💡 chord_spatial 时建议关闭（弦距采样自身消除噪声弧长累积）
# 💡 adaptive_spatial 时建议开启（弧长插值需预平滑压制噪声）
# 💡 与 tunable_registration.py 保持一致！
PRE_SMOOTH = {
    "enable": True,                # ⭐ chord_spatial时关闭，adaptive_spatial时建议开启
    "method": "gaussian",
    "gaussian_sigma": 5.0,
    "bspline_smoothing": 3.0,
    "bspline_k": 3,
}
...
PREPROCESS_CONFIG = {
    ...
    "gaussian_sigma": 3.0,          # ⭐ 后平滑sigma（重采样后应用，用于最终配准）
...
def transform_for_verification(source_csv, target_csv, transform_path=None, 
    ...
    if mls_transform is not None:
        ...
        # 应用MLS变换
        transformed_source = mls_transform.transform_trajectory(source_points)
        target_smoothed = target_points
        
        if verbose:
            print(f"  ⭐ MLS变换完成（{len(transformed_source)}点）")
...
def transform_csv(input_csv, output_csv, transform_path=None, xyz_cols=(3, 4, 5), 
    ...
    if method == "segmented" and mls_transform is not None:
        ...
        normalized_arc = arc_lengths / total_arc
        
        if verbose:
            print(f"  轨迹弧长: {total_arc:.2f}mm")
        
        # 对每个点应用MLS变换（使用原始点坐标 + 预平滑弧长位置）
        use_mode = mls_transform.recommended_mode
        ...
        if use_mode == "grid" and mls_transform.grid_transforms is not None:
            for i in range(len(points_original)):
                transformed[i] = mls_transform.transform_point_grid(points_original[i], normalized_arc[i])
        else:
            for i in range(len(points_original)):
                transformed[i] = mls_transform.transform_point_full(points_original[i], normalized_arc[i])
        
        if verbose:
            print(f"  ⭐ MLS变换完成（{len(transformed)}点, 模式={use_mode}）")
...
def interactive_mode():
    ...
    if not input_csv:
        input_csv = r"C:\Users\15421\Desktop\lighthouse_1.12\Assets\StreamingAssets\TrackerRecordings\tracker1 - tcp.csv"
...
def main():
    ...
    if len(sys.argv) == 1:
        # 无参数，自动使用默认配置运行
        default_input = r"C:\Users\15421\Desktop\lighthouse_1.12\Assets\StreamingAssets\TrackerRecordings\tracker6 - tcp.csv"
        default_output = r"C:\Users\15421\Desktop\lighthouse_1.12\Assets\StreamingAssets\TrackerRecordings\tracker6 - tcp_transformed.csv"
